"""Learning-rate scheduler helpers used by train_fusion.py."""
...
def build_scheduler(optimizer, name="warmup_cosine", **kwargs):
    name = str(name).lower()
    if name == "none":
        return None
    ...
    if name == "warmup_cosine":
        ...
        warmup_steps = int(
            float(kwargs.get("warmup_epochs", 1)) * steps_per_epoch
        )
        ...
        def lr_multiplier(step):
            ...
            progress = min(
                max((step - warmup_steps) / decay_steps, 0.0), 1.0
            )
...
def step_scheduler(scheduler, name, val_loss=None):
    if scheduler is None:
        return
    if str(name).lower() == "reduce_on_plateau":
